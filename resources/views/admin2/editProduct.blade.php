...
@section('content')
    <div class="container mt-2">
        <form action="{{ route('admin2.product.update', $product->id) }}" method="POST">
            @csrf
            @method('patch')

            <div class="row">
                <div class="col-md-6">
                    <img src="{{ asset('/assets/images/dynamic/' . $product->image_path) }}" alt="{{ $product->nom_P }}"
                        class="img-fluid">
                </div>
                <div class="col-md-6">
                        ...
                        <select name="id_Categorie" id="id_Categorie" class="form-control" required>
                            ...
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}"
                                    {{ old('id_Categorie', $product->id_Categorie) == $category->id ? 'selected' : '' }}>
                                    {{ $category->nom_C }}</option>
                            @endforeach
                        </select>
                    ...
                    <center>
                        <button type="submit" class="btn btn-primary">Modifier <i class="uil uil-edit-alt"></i></button>
                    </center>
                </div>
            </div>
            <hr>
                                ...
                                <div class="row">
                                    <div class="col">
                                        <input type="text" id="variant_{{ $variant->id }}_valeur"
                                            name="variant[{{ $variant->id }}][valeur][]"
                                            value="{{ $productVariant->valeur ?? '' }}" class="form-control">
                                    ...
                                    <div class="col">
                                        <input type="number" id="variant_{{ $variant->id }}_prix"
                                            ...
                                            value="{{ $productVariant->prix ?? '' }}" class="form-control">
                                    </div>
            ...
            <button type="submit" class="btn btn-primary">Update product <i class="uil uil-edit-alt"></i></button>
