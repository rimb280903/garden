                    ...
                    <div class="card-body">
                        <form method="POST" action="{{route('admin.product.store')}}" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select id="id_Categorie" class="form-control @error('id_Categorie') is-invalid @enderror" name="id_Categorie" required>
                                  <option value="">{{ __('Select Category') }}</option>
                                  @foreach ($categories as $category)
                                      <option value="{{ $category->id }}" {{ old('id_Categorie') == $category->id ? 'selected' : '' }}>
                                          {{ $category->nom_C }}
                                      </option>
                                  @endforeach
                              </select>
                            ...
                            <div class="d-grid gap-2 d-flex justify-content-center  ">
                                <button type="submit" class="btn btn-primary">{{ __('Ajouter Produit') }}</button>
                            </div>
                        </form>
                    </div>
                </div>
